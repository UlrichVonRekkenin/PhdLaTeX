\chapter{По мотивам CIM часть 2}
...
и защищенный метод \texttt{IModbusElementWriter::write()}.
\lstinputlisting[
    language=C++,
...
Как видно из рисунка \ref{fig:modbus_class_uml} правила записи описываются с помощью компонента
\texttt{ModbusDataRelationed}, который является наследником класса \texttt{ModbusData}.
Этот класс содержит в себе \texttt{value} от родительского класса и правило отношения \texttt{Relation}
как поле класса. Экземпляр этого класса используется в уточнении функции \texttt{IModbusElementWriter::conditionsMet()}
у наследников 
\lstinputlisting[
    language=C++,
    caption=Уточнение метода в реализации класса \texttt{ModbusGuardedWriter} из \ref{fig:guard}
        ]{Dissertation/listings/cpp/conditionsMet.hpp}
