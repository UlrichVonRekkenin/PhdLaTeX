% Создать в рамках производства сложных критичных изделий 
% создать имитатор, который \dots
% Имитационный комплекс 

% Пространство параметров... это и есть онтология.
% Которые связаны между собой.

% Прогнозировать поведение объекта.

% Запросить статьи про "переход от онтологии к коду".
...
модели $s_i$, $s_j$ будут изоморфны\footnote{Например, напряжение и ток двигателя появляется только при выполнении условия $R1 = \mbox{true}$}.
...
области\footnote{Например, нельзя обуславливать срабатывание узлов PP\_1 и PP\_2 наличием напряжения на двигателе.}.
...
в таблице \ref{tbl:meta_data_levels} \cite{book:gost:56272,w3c:rdf:lassila}.
\begin{table}[hb!]
    \begin{center}
    ...
    1 & Класс & Класс(Напряжение, [поле (имя-строка, значение-число, адрес-число)]) \\\hline
    0 & Объект & Напряжение(Приборное, 27В, 42) \\\hline
\end{tabular}
\end{center}
\end{table}

Необходимость разработки онтологии связана с:
\begin{enumerate}
...
\begin{table}[hb!]
\begin{center}
\caption{\todo{Сводная таблица наследников}}\label{tbl:ModbusElementWriterImpl}
